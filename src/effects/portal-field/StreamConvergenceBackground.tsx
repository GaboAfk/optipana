...
export function StreamConvergenceBackground({ className = "", ...props }: StreamConvergenceBackgroundProps) {
  const hostRef = useRef<HTMLDivElement>(null), canvasRef = useRef<HTMLCanvasElement>(null); const optionsRef = useRef({ ...STREAM_CONVERGENCE_DEFAULTS, ...props });
  useEffect(() => { optionsRef.current = { ...STREAM_CONVERGENCE_DEFAULTS, ...props }; });
  useEffect(() => {
  ...
  const options = { ...STREAM_CONVERGENCE_DEFAULTS, ...props }; return <div ref={hostRef} className={`threeui-background stream-convergence${className ? ` ${className}` : ""}`}><canvas ref={canvasRef} style={{ opacity: options.opacity, filter: `hue-rotate(${options.hue}deg) saturate(${options.saturation}) brightness(${options.brightness})`, transform: `scale(${options.scale})` }} /></div>;
}
